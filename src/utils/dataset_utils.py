...
def deduplicate_smiles(df, rule, smiles_col='standardized_smiles'):
    """
    Deduplicate a DataFrame based on a SMILES column using a custom rule.
    rule should take a DataFrame (group) and return a single row (Series).
    """
...
def merge_and_deduplicate(dfs, smiles_col, rule):
    ...
    logger.success(f"Merged dataset has {len(merged)} entries; after deduplication, {len(deduped)} unique entries remain. (Removed {len(merged) - len(deduped)} duplicates.)")
    return deduped