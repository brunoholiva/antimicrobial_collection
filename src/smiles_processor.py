...
class DataFrameSmilesProcessor:
    ...
    def process(self, df: pd.DataFrame, smiles_column: str) -> pd.DataFrame:
        """
        Takes a DataFrame with a column of SMILES strings, validates, standardizes,
        ...
        Returns:
            A processed DataFrame with valid, standardized, and unique SMILES strings.
        """
        ...
        processed_df["is_valid"] = processed_df[smiles_column].apply(
        ...
        logger.success(
            f"SMILES validation completed. {len(valid_df)} valid entries retained."
        )

        # 2. Standardize SMILES
        logger.info(f"Starting SMILES standardization of {len(valid_df)} entries.")

        valid_df["standardized_smiles"] = valid_df[smiles_column].apply(
            self.standardizer.standardize_smiles
        )
        valid_df["is_valid"] = valid_df["standardized_smiles"].apply(
            self.validator.is_valid
        )
        ...
        logger.info(
            f"Number of invalid SMILES after standardization: {num_invalid_after_std}"
        )

        valid_df = valid_df[valid_df["is_valid"]].copy()

        logger.success(
            f"SMILES standardization completed. {len(valid_df)} valid entries retained."
        )
        ...
        logger.success(
            f"Duplicate removal completed. {len(valid_df)} unique entries retained, from {len(processed_df)} original entries."
        )
